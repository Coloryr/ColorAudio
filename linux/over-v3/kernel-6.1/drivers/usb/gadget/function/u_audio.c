...
static int get_max_size(const int *size)
{
	int i, max_size = 0;
...
static int uac_pcm_open(struct snd_pcm_substream *substream)
{
	...
	audio_dev = uac->audio_dev;
	params = &audio_dev->params;
	p_ssize = params->p_ssizes[audio_dev->in_alt - 1];
	c_ssize = params->c_ssizes[audio_dev->out_alt - 1];
...
int u_audio_start_playback(struct g_audio *audio_dev)
{
	...
	uac->p_framesize = get_max_size(params->p_ssizes) *
