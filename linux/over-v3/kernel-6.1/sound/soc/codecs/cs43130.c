...
static int cs43130_pll_config(struct snd_soc_component *component)
{
	...
	dev_info(component->dev, "cs43130->mclk = %u, cs43130->mclk_int = %u\n",
			   ...
			   unsigned int freq_in, unsigned int freq_out)
{
	...
	dev_info(component->dev, "cs43130->pll_bypass = %d", cs43130->pll_bypass);
	return ret;
}
...
static int cs43130_pcm_set_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	dev_info(component->dev, "dai_id = %d,  dai_mode = %u, dai_format = %u\n",
		codec_dai->id,
...
static int cs43130_dsd_set_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	dev_info(component->dev, "dai_mode = 0x%x\n",
		cs43130->dais[codec_dai->id].dai_mode);
				  ...
				  int clk_id, unsigned int freq, int dir)
{
	...
	dev_info(component->dev, "dai_id = %d,  sclk = %u\n", codec_dai->id,
				 ...
				 struct cs43130_private *cs43130)
{
	...
	if (msk == CS43130_HPLOAD_DC_INT) {
		...
		dev_info(component->dev, "HP DC impedance (Ch %u): %u\n", !left_ch,
			impedance);
	} else {
		if (left_ch)
		...
		dev_info(component->dev, "HP AC (%u Hz) impedance (Ch %u): %u\n",
			       ...
			       unsigned int rslt_msk, int ac_idx)
{
	...
	dev_info(component->dev, "HP load stat: %x, INT_MASK_4: %x\n",
	...
	if ((cs43130->hpload_stat & (CS43130_HPLOAD_NO_DC_INT |
		...
		dev_info(component->dev, "HP load measure failed\n");
		return -1;
	}
...
static void cs43130_imp_meas(struct work_struct *wk)
{
	...
	dev_info(component->dev, "Set HP output control. DC threshold\n");
	for (i = 0; i < CS43130_DC_THRESHOLD; i++)
		dev_info(component->dev, "DC threshold[%d]: %u.\n", i,
...
static irqreturn_t cs43130_irq_thread(int irq, void *data)
{
	...
	dev_info(component->dev, "number of interrupts occurred (%u)\n",
		irq_occurrence);
	...
	if (stickies[3] & CS43130_HPLOAD_AC_INT) {
		cs43130->hpload_stat = stickies[3];
		dev_info(component->dev, "HP AC load measurement done (%x)\n",
			cs43130->hpload_stat);
		complete(&cs43130->hpload_evt);
		return IRQ_HANDLED;
	}
	...
	if (stickies[3] & CS43130_HPLOAD_DC_INT) {
		cs43130->hpload_stat = stickies[3];
		dev_info(component->dev, "HP DC load measurement done (%x)\n",
			cs43130->hpload_stat);
		complete(&cs43130->hpload_evt);
		return IRQ_HANDLED;
	}
	...
	if (stickies[3] & CS43130_HPLOAD_ON_INT) {
		cs43130->hpload_stat = stickies[3];
		dev_info(component->dev, "HP load state machine on done (%x)\n",
			cs43130->hpload_stat);
		complete(&cs43130->hpload_evt);
		return IRQ_HANDLED;
	}
	...
	if (stickies[3] & CS43130_HPLOAD_OFF_INT) {
		cs43130->hpload_stat = stickies[3];
		dev_info(component->dev, "HP load state machine off done (%x)\n",
			cs43130->hpload_stat);
		complete(&cs43130->hpload_evt);
		return IRQ_HANDLED;
	}
	...
	if (stickies[0] & CS43130_HP_UNPLUG_INT) {
		dev_info(component->dev, "HP unplugged\n");
		cs43130->hpload_done = false;
	...
	if (stickies[0] & CS43130_HP_PLUG_INT) {
		if (cs43130->dc_meas && !cs43130->hpload_done &&
			...
			dev_info(component->dev, "HP load queue work\n");
