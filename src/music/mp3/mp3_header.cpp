...
static int parse_frame_header(ColorAudio::Stream *st, MP3FrameHeader *hdr)
{
    ...
    if (st->read(buffer, sizeof(buffer)) != 4)
    {
        return 0;
    }

    // 组合同步字 (11位)
    uint16_t sync = ((buffer[0] << 3) | (buffer[1] >> 5)) & 0x7FF;

    // 验证同步字
    if (sync != 0x7FF)
    {
        st->seek(-3, SEEK_CUR); // 回退3字节
        return 0;
    }

    // 解析帧头字段
    hdr->sync = sync;
    hdr->version = (buffer[1] >> 3) & 0x03;
    hdr->layer = (buffer[1] >> 1) & 0x03;
    hdr->private_bit = buffer[1] & 0x01;
    hdr->bitrate_index = buffer[2] >> 4;
    hdr->sample_rate_idx = (buffer[2] >> 2) & 0x03;
    hdr->padding = (buffer[2] >> 1) & 0x01;
    hdr->private_bit = buffer[2] & 0x01;
    hdr->channel_mode = buffer[3] >> 6;
...
float mp3_get_time_len(ColorAudio::Stream *st)
{
    ...
    if (parse_frame_header(st, &hdr))
    {
        ...
        {
            // CBR模式处理 - 逐帧扫描
            st->seek(pos, SEEK_SET);
            uint32_t size = st->get_all_size() - pos;

            while (st->can_read())
            {
                if (!parse_frame_header(st, &hdr))
                {
                    st->seek(1, SEEK_CUR); // 移动1字节继续搜索
                    continue;
                }
