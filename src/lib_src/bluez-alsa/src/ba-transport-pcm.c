	...
	enum ba_transport_pcm_state state)
{
	...
	if (state == BA_TRANSPORT_PCM_STATE_TERMINATED && (old_state == BA_TRANSPORT_PCM_STATE_IDLE ||
													   old_state == BA_TRANSPORT_PCM_STATE_STARTING))
		valid = true;
	...
	if (state != old_state && (state == BA_TRANSPORT_PCM_STATE_RUNNING ||
...
int ba_transport_pcm_volume_sync(struct ba_transport_pcm *pcm, unsigned int update_mask)
{
	...
	if (pcm->soft_volume && t->profile & (BA_TRANSPORT_PROFILE_A2DP_SOURCE | BA_TRANSPORT_PROFILE_MASK_AG))
		goto final;
