...
{
    lv_volume_click(volume_pan_obj);
}
                               ...
                               lv_event_cb_t next, lv_event_cb_t mute)
{
    ...
    lv_wave_images_create(cont, 0);
    lv_wave_images_create(cont, 1);
    ...
    lv_obj_set_style_shadow_width(image_bg, 12, 0);
    lv_obj_set_style_shadow_spread(image_bg, 5, 0);
    ...
    volume_pan_obj = lv_volume_create(cont, volume, mute);
    lv_obj_align_to(volume_pan_obj, image_bg, LV_ALIGN_OUT_RIGHT_MID, 30, 0);

#if LV_MUSIC_DEBUG
    lv_obj_set_style_bg_color(title_box, lv_color_hex(0x00FF00), 0);
    lv_obj_set_style_bg_opa(title_box, 80, 0);

    lv_obj_set_style_bg_color(lyric_obj, lv_color_hex(0xFF0000), 0);
    lv_obj_set_style_bg_opa(lyric_obj, 80, 0);

    lv_obj_set_style_bg_color(ctrl_box, lv_color_hex(0x0000FF), 0);
    lv_obj_set_style_bg_opa(ctrl_box, 80, 0);
#endif
    return main_cont;
}
...
void lv_music_set_sound_info(uint16_t bit, uint32_t rate, uint8_t channel, uint32_t bps, bool isflac)
{
...
void lv_music_set_play_mode(bool isrnd)
{
    if (!isrnd)
    {
        lv_image_set_src(play_mode_obj, &img_lv_demo_music_btn_loop);
...
{
    lv_volume_set_value(volume_pan_obj, (int32_t)value);
}
