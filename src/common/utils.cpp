...
bool load_image(uint8_t *data, uint32_t size, lv_image_dsc_t *img_dsc)
{
    ...
    else if (png_sig_cmp(data, 0, 8) == 0)
    {
        ...
        img_dsc->data = static_cast<png_bytep>(malloc(img_dsc->data_size));
        if (!img_dsc->data)
        {
            LV_LOG_ERROR("Png decode error");
        ...
        png_bytepp row_pointers = static_cast<png_bytepp>(malloc(height * sizeof(png_bytep)));
